...
#sfunc _on_hold_button_mouse_exited() -> void:
#	mouse_inside = false
...
func on_meter_full() ->void:
	...
	AudioHandler.shotgun_reload_success.play()
	# the animation
...
func _on_v_scroll_bar_value_changed(value: float) -> void:
	if value <=10 && !moving_up && moved_down:
		AudioHandler.shotgun_reload_engage.play()
		meter_charge_rate.start()
		moving_up = true
	elif value > 10:
		meter_charge_rate.stop()
