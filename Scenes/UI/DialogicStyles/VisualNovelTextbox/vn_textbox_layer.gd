...
@export var next_indicator_enabled: bool = false
...
func _apply_export_overrides() -> void:
	...
	Dialogic.Text.about_to_show_text.connect(on_text)
...
const CMDR_CLOAKE_BOX = preload("res://Assets/Sprites/HUD/Dialogue/CMDR Cloake box.png")
const PILOT_CIRCE_BOX = preload("res://Assets/Sprites/HUD/Dialogue/Pilot circe box.png")
const DIALOGUE_BOX_NO_NAME = preload("res://Assets/Sprites/HUD/Dialogue/dialogue_box_no_name.png")
@onready var dialogue_box: TextureRect = $Dialogue_box

const ANKLEPANTS = preload("res://Assets/Fonts/Anklepants.otf")
#const ARCHLUX_SAN_MEDIUM = preload("res://Assets/Fonts/archluxSanMedium.ttf")

const AKASHA_ITALIC_BW_DD_8 = preload("res://Assets/Fonts/AkashaItalic-BWDd8.ttf")
const AKASHA_REGULAR_RPRN_6 = preload("res://Assets/Fonts/AkashaRegular-Rprn6.ttf")

const WARRIOT_ROUND_ITALIC = preload("res://Assets/Fonts/WarriotRound-Italic.ttf")
const WARRIOT_ROUND_REGULAR = preload("res://Assets/Fonts/WarriotRound-Regular.ttf")

@onready var dialogic_node_dialog_text: DialogicNode_DialogText = %DialogicNode_DialogText
...
func on_speaker_updated(character: DialogicCharacter)->void:
	...
	if character == null:
		dialogue_box.texture = DIALOGUE_BOX_NO_NAME
		dialogic_node_dialog_text.add_theme_font_size_override("normal_font_size",20)
		dialogic_node_dialog_text.add_theme_font_size_override("italics_font_size",20)
		dialogic_node_dialog_text.add_theme_font_override("normal_font",ANKLEPANTS)
		dialogic_node_dialog_text.add_theme_font_override("normal_font",ANKLEPANTS)
		return
	...
	dialogic_node_dialog_text.add_theme_font_size_override("normal_font_size",35)
	dialogic_node_dialog_text.add_theme_font_size_override("italics_font_size",35)
	dialogic_node_dialog_text.add_theme_font_override("normal_font",AKASHA_REGULAR_RPRN_6)
	dialogic_node_dialog_text.add_theme_font_override("italics_font",AKASHA_ITALIC_BW_DD_8)
	if character.get_character_name() == "cmdr_cloake":
		dialogue_box.texture = CMDR_CLOAKE_BOX
	elif character.get_character_name() == "pilot_circe":
		dialogue_box.texture = PILOT_CIRCE_BOX
		
		
		
func on_text( text)->void:
# { "text": " Banging my head on the screen, what an introduction.", "character": <null>, "portrait": "", "append": true }
	if text["character"] ==null:
		return
	if text["append"] == true:
		return
	
	if GlobalVar.dialogue_play_static:
		AudioHandler.radio.play()
